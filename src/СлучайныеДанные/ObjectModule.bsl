...
//   Строка
////
Функция СлучайныйСимвол(ГенераторСлучайныхЧисел = Неопределено, Знач Параметры = Неопределено) Экспорт 
	
	Если Параметры = Неопределено Тогда
		Параметры = ПолучитьСтруктуруПараметровГСС();
	КонецЕсли;

	МассивСимволов = ПолучитьМассивСимволов(Параметры);
	СлучайноеЧисло = СлучайноеЧисло(ГенераторСлучайныхЧисел, 0, МассивСимволов.Количество() - 1);

	Возврат МассивСимволов[СлучайноеЧисло];
...
////
Функция ПолучитьСтруктуруПараметровГСС(Числа = Истина, РусскиеБуквы = Истина, РусскиеБуквыЗаглавные = Истина, АнглийскиеБуквы = Истина, АнглийскиеБуквыЗаглавные = Истина, СпециальныеСимволы = Истина, Пробел = Ложь) Экспорт 
	...
	Структура.Вставить("СпециальныеСимволы", СпециальныеСимволы);
	Структура.Вставить("Пробел", Пробел);
...
////
Процедура УстановитьМаксимальноеКоличество(Количество) Экспорт
...
//
// Параметры: 
//   Структура (Структура) - Струкрута параметров
//
// Возвращаемое значение: 
//   Массив [ Строка ] 
////
Функция ПолучитьМассивСимволов(Параметры) 
	
	Массив = Новый Массив;

	Если ПолучитьЗначениеСтруктуры(Параметры, "Числа", Ложь) Тогда
		Для Н = 48 По 57 Цикл
			Массив.Добавить(Символ(н));
		КонецЦикла;
	КонецЕсли;

	Если ПолучитьЗначениеСтруктуры(Параметры, "РусскиеБуквы", Ложь) Тогда
		Для Н = 1072 По 1103 Цикл
			Массив.Добавить(Символ(н));
		КонецЦикла;

		Массив.Добавить(Символ(1105));
	КонецЕсли;

	Если ПолучитьЗначениеСтруктуры(Параметры, "РусскиеБуквыЗаглавные", Ложь) Тогда
		Для Н = 1040 По 1071 Цикл
			Массив.Добавить(Символ(н));
		КонецЦикла;

		Массив.Добавить(Символ(1025));
	КонецЕсли;

	Если ПолучитьЗначениеСтруктуры(Параметры, "АнглийскиеБуквы", Ложь) Тогда
	...
	Если ПолучитьЗначениеСтруктуры(Параметры, "СпециальныеСимволы", Ложь) Тогда
		Для Н = 33 По 47 Цикл
			Массив.Добавить(Символ(н));
		КонецЦикла;
	...
	Если ПолучитьЗначениеСтруктуры(Параметры, "Пробел", Ложь) Тогда
		Массив.Добавить(" ");
	КонецЕсли;
...
//
// Параметры: 
//   Структура (Структура) - Струкрута данных
//   Ключ (Строка) - Ключ
...
//   Произвольный / Неопределено
////
Функция ПолучитьЗначениеСтруктуры(Структура, Ключ, ЗначениеПоУмолчанию = Неопределено)
	
	НайденноеЗначение = Неопределено;
	
	Если Структура.Свойство(Ключ, НайденноеЗначение) Тогда
